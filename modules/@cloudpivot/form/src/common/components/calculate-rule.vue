  ...
  >
    <div class="calculate-rule">
      ...
      <div class="text-area">
        ...
        ></a-textarea>-->
        <div ref="ruleHTMLRef" :id="contentId" class="ant-input" contenteditable="true"></div>
      </div>
    </div>
  </a-modal>
</template>
...
export default class CalculateRule extends Vue {
  ...
  created() {
    ...
    for(let key of allCtrlKeys) {
      ...
      if(item.type === FormControlType.Number && item.key !== code){
        this.dataItems.push({
  ...
  testRuleString(str: string) {
    ...
    // 校验输入的内容格式是否正确
    var sss = str.replace(/(avg)|(max)|(min)|(sum)|(count)/g, "C"); // 将方法标记为 C 子表数据可以用于 所有公式
