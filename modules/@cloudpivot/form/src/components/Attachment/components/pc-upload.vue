<template>
  ...
  >
    ...
    <div class="message" v-if="!isReadonly && controlOpts.onlyShoot">
      请在手机端拍照上传
    </div>
...
export default class PcUpload extends FileUploadControl {
  ...
  onChange(info: { file: any; fileList: any[]; event: any }): void {
    ...
    this.uploadFileList = info.fileList.filter(
      (f) =>
        f.status &&
        !(f.status === UploadStatus.Delete || f.status === "removed")
    );
    //保存文件状态
    let { file } = info;
    ...
    console.log(this.fileList, "ddddddddddd");
  }
  /**
   * @des 删除文件
   */
  ...
  handlePreview(file: any) {
    // 附件预览功能暂时失效，以后预览功能正式接入后可放开限制，直接使用附件预览接口
    console.log("handlePreview file...", file.previewUrl);
    if (file.previewUrl) {
