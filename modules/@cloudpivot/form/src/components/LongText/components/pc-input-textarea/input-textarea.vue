...
<template>
  <div>
    <template v-if="!readonly">
      <h3-textarea v-if="isLangText" v-model="val" :placeholder="placeholder" :autosize="rows" :maxLength="maxLength" :disabled="readonlyFormula" @change="onChange"></h3-textarea>
      <div v-else :class="{'disabled': readonlyFormula}">
        <h3-editor @change="onChange" v-model="val" :locale="locale" :disabled="readonlyFormula" :id="`${id}editor`" />
      </div>
    </template>
