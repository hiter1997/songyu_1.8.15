  ...
  <div class="form-approval">
    <div v-for="(approval, i) in approvalsData" :key="i" class="form-approval__item">
      ...
      <div v-if="approval.nodes && approval.nodes.length" class="form-approval__instances">
        <div v-for="(node, g) in approval.nodes" :key="g" :class="`form-approval__instance${approval.subInstanceActivity ? ' form-approval__instance--sub' : ''}`" @click="clickApproval(node, approval.subInstanceActivity)" v-show="approval.subInstanceActivity || (node.workItemStatus !== 1 && node.workItemStatus !== 2) || node.approvaler.length">
