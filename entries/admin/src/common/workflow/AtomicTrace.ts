...
export default class AtomicTrace {
  constructor(_TraceType: number, _Target: any, _Source: any) {
    ...
    let _activities = workflowStoreData.activities;
    switch (_TraceType) {
      case TraceType.Activity.Remove:
        {
          ...
          this.Redo = () => {
            ...
            _lines.forEach((m: any) => { // mutiple lines need to be removed 
      ...
      case TraceType.Activity.Add:
        {
          ...
          this.Undo = () => {
            ...
            //   this.Activity.Unselect();
            // }
            _activities.some((activity: WorkflowNamespace.Activity, idx: number) => {
              if (activity.ID === _ID) {
                _index = idx;
                return true;
              }
