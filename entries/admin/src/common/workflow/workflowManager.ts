...
export default class WorkflowManager extends WorkflowBase {
  ...
  setSameStyle(sameStyle: string) {
    ...
    if (workflowStoreData.selectedActivities && workflowStoreData.selectedActivities.length > 1) {
      const ObjectAndStates = TraceManager.GetMultiObjAndStates(workflowStoreData.selectedActivities);
        ...
        case WorkflowNamespace.SameStyle.Width:
          {
            ...
            workflowStoreData.selectedActivities.forEach((activity: WorkflowNamespace.Activity, index: number) => {
              if (index != 0) {
      ...
      // 重新绘线
      redrawOnActivityChange(workflowStoreData.selectedActivities, workflowStoreData.lines);

      // workflow.autoFit();
      workflowStoreData.traceManager.AddTrace(_TraceType, workflowStoreData.selectedActivities, workflowStoreData.beforeMovedActivities);
    }
  }
}
