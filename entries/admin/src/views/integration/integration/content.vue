  ...
  <div>
    ...
    <div class="integration__table">
      ...
      >
        ...
        <span slot="action" slot-scope="text,record" class="integration__actions">
          ...
          <i
            v-if="isRestful(record) || isDatabase(record)"
          ...
          />
          <!-- 运行webserver -->
          <i v-else class="icon aufontAll h-icon-all-play-circle-o" @click="runItem(record)" />
          <!-- 删除服务条目 -->
    ...
    >
      <div class="refer-modal">
        ...
        <refer-method :serviceMethod="currentItem" :showTitle="false" :showRoute="true" />
      </div>
    </a-modal>
  </div>
</template>
...
export default class IntegrationContent extends Vue {
  ...
  isCanExpand(record: BizService.Service.Item) {
    return /restful|soap|data_/i.test(record.adapterCode)
  }
  ...
  addMethod(record: BizService.Service.Item) {
    ...
    if (/data_sql/i.test(record.adapterCode)) {
      // 数据库服务
  ...
  editMethod(method: BizService.Method.Item, service: BizService.Service.Item) {
    ...
    if (/data_sql/i.test(service.adapterCode)) {
      // 数据库服务
