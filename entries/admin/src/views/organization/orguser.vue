<template>
  <div class="org-user">
    <a-tabs default-active-key="1" @change="tabChange" class="user-tabs">
      <a-tab-pane key="1" tab="在职人员">
        <div class="user-tree" v-resize.east>
          ...
          <div class="search-wrap">
            ...
            >
              ...
              <a-icon class="suffix-icon" type="search" slot="suffix" @click="onSearch" />
            </a-input>
          </div>

          <!-- 组织树 -->
          <org-tree-component @selectNodeEvent="selectNode" ref="orgTree"></org-tree-component>
        </div>
        <div class="user-table">
          <div class="user-title search__input">
            <h2 v-show="!isSearchTitle">
              ...
              >
                <template slot="content">
                  <p v-show="btnsDisplayPermission.copyID" @click="copyID(curNodeState.id)">复制 ID</p>
                  <p v-show="btnsDisplayPermission.edit" @click="showDepartment(1, curNodeState)">编辑</p>
                  <p v-show="btnsDisplayPermission.deleteAble" @click="deleteDepartment">删除</p>
                </template>
            ...
            <div class="add-operation" v-if="!isSearchTitle">
              ...
              <a-dropdown v-show="personImportExport || deptPersonSync">
                <a-button>
                  <i class="icon aufontAll h-icon-all-ellipsis-o op-icon"></i> 更多
                </a-button>
                  ...
                  <div class="btns-group">
                    <template v-show="personImportExport">
                      <div
                        ...
                        class="like-btn"
                        @click="importPerson"
                      >{{ $t("languages.ImportPerson") }}</div>
                      <div
                        v-show="btnsDisplayPermission.exportPerson"
                        class="like-btn"
                        @click="exportPerson"
                      >{{ $t("languages.ExportPerson") }}</div>
                    </template>
          ...
          <div v-show="orgUserList.length" class="table-wrap" :class="{'search-wrap': isSearchTitle}">
            <a-table
              class="user-list"
            ...
            >
              ...
              ><span class="line"></span>{{ $t("languages.NO") }}</span>
              <span slot="nameTitle" class="resize"><span class="line"></span>{{ $t("languages.Name") }}</span>
              <span slot="roleNameTitle" class="resize"><span class="line"></span>{{ $t("languages.Roles") }}</span>
              <span slot="mobileTitle" class="resize"><span class="line"></span>{{ $t("languages.User.MobilePhone") }}</span>
              <span slot="emailTitle" class="resize"><span class="line"></span>{{ $t("languages.User.Email") }}</span>
              <span slot="operationTitle" class="resize"><span class="line"></span>{{ $t("languages.Operation") }}</span>
              ...
              <p slot="name" class="cursor text-ellipsis" slot-scope="text, record">
                <a-tooltip placement="top">
                  ...
                  <span class="name-span" v-if="searchKey">
                    <template
              ...
              <span slot="mobile" class="text-ellipsis" slot-scope="text">{{ text }}</span>
              <span slot="email" class="text-ellipsis" slot-scope="text">{{ text }}</span>
              <div
                slot="operation"
              ...
              :trigger="['hover']"
                ...
                <i
                  ...
                  class="cursor icon aufontAll h-icon-all-edit-o"
                  @click.self.stop="showUser(1, record)"
                ></i>
                ...
                >
                  ...
                  <a-menu slot="overlay">
                    ...
                    <a-menu-item key="1" v-if="record.relatedSyncType === 'PUSH'" @click="deleteUser(record.id)">
    ...
    >
      <UserInfo @closeDrawer="onCloseInfoModal"></UserInfo>
    </a-drawer>
    ...
    >
      ...
      <div class="modal-footer">
        ...
        <a-button v-if="showReImport" key="import" type="primary" @click="reImport">重新导入</a-button>
        <a-button
    ...
    <a-modal
      ...
      :maskClosable="false"
    >
      <staff-selector :options="deptOpts" v-model="syncDepartSelected" :params="filterDept"></staff-selector>
    </a-modal>
    <!-- 同步部门 ]] -->
    <!-- 
...
export default class OrgUser extends  mixins(OrgMixins) {
  ...
  async setDisplayOnClickDept(curNodeState: TreeNode) {
    // 判断是否有关联组织 调接口判断 arr  []  len < 2 true
    let isSingleCrop: boolean = true;

    const allCropRes: any = await OrgApi.getOrgaList();
    if (allCropRes.errcode === 0) {
      const { data } = allCropRes;
      isSingleCrop = data.length < 2;
    }
    ...
    OrgApi.getDeptInfo(deptInfoParams).then((res: any) => {
      if (res.errcode === 0) {
        ...
        this.isSelfMaintainRootNode = res.data.relatedSyncType === 'PUSH' && res.data.isCorpRootNode;
        this.curDeptParentId = res.data.parentId || '';

        /******** 按钮展示逻辑 start *********/
        const { isRoot } = curNodeState;
        const { relatedSyncType, isCorpRootNode, displayOption, relatedOrgType } = res.data;
        ...
        this.btnsDisplayPermission.deleteAble = isPushOrg && !isCorpRootNode && !isRoot;
        ...
        this.btnsDisplayPermission.syncDepartment = isPushOrg && !isMainOrg && displayOption;
        this.btnsDisplayPermission.syncPerson = isPushOrg && !isMainOrg && displayOption;

        this.btnsDisplayPermission.createUser = isPushOrg && displayOption;
        this.btnsDisplayPermission.createDepartment = isPushOrg && displayOption;
        /******** 按钮展示逻辑 end **********/
  ...
  async syncPeople() {
    ...
    if (errcode === 0) {
      return this.$message.success(errmsg);
    }
