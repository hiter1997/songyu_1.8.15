    ...
    <div class="role-tree" v-resize.east>
      ...
      >
        <a-tree-node
          ...
          :nodeData="node"
        >
          <span slot="title" :title="node.title" class="role-title">
            {{ node.title }}
            <span class="dingding" v-if="node.roleType === 'DINGTALK'"
              >钉钉</span
            >
            <a-popover
              placement="rightTop"
          ...
          <template v-if="node.children">
            <a-tree-node
              ...
              :nodeData="child"
            >
              <span slot="title" :title="child.title" class="role-title">
                {{ child.title }}
                <a-popover
                  placement="rightTop"
                  trigger="click"
                  :visible="isShowPop && iconCode === child.id"
                  overlayClassName="role-popover"
                  @visibleChange="onVisibleChange($event, child.id)"
                >
                  <template slot="content">
                  ...
                  <i
                    v-if="node.roleType === 'SYS'"
                    class="icon aufontAll h-icon-all-setting-o edit"
                    :class="{ isshow: isShowPop && iconCode === child.id }"
                    @click.stop="() => {}"
                  ></i>
                </a-popover>
              </span>
    ...
    <div class="role-content">
      <div class="role-content__search">
        <div class="search__role">
          <template v-if="userInfo.code">
            <span class="search__role_txt" :title="userInfo.name">{{ `${userInfo.name}` }}</span> &nbsp;&nbsp;&nbsp;<span>{{
              `ID：${userInfo.code}`
            }}</span></template
          >
        </div>
        <div class="search__input">
          <a-button :disabled="isBatchOperate" @click="showUser(2)" class="batch-edit-btn"><i class="icon aufontAll h-icon-all-list-work-o list-work-icon"></i>批量编辑</a-button>
          <a-input
            class="seach-input"
      ...
      <div class="table-wrap">
        ...
        >
          ...
          <span slot="nameTitle" class="resize"><span class="line"></span>用户</span>
          <span slot="departmentTitle" class="resize"><span class="line"></span>所属部门</span>
          <span slot="ouScopeTitle" class="resize"><span class="line"></span>{{
      ...
      <div class="load-more">
        ...
        <template v-if="!isDataEmpty">
          ...
          <span class="text" v-if="isLoadMore">{{
...
export default class orgrole extends Vue {
  ...
  get indeterminate() {
    ...
    const checkedLength: number = this.roleList.filter((list: any) => list.checked).length;
