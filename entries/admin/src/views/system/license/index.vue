  ...
  <div class="license">
    ...
    >
      <div class="au-box">
        <p
          v-if="isOver"
          class="over"
        >检测到您当前授权用户数超过限制（最大限制{{ permObj.maxAuthorizedUserNum }}人，当前{{ permObj.authorizedUserNum }}人，超出{{ permObj.authorizedUserNum - permObj.maxAuthorizedUserNum }}人），系统已经停止服务，减少授权用户数后才能恢复服务</p>
        ...
        <div class="choose-part" v-show="!isAllowAll">
          ...
          <div class="choose-item">
            ...
            <div class="choose-con">
              <staff-selector v-model="dept" :options="taffSelectorOptsDept"></staff-selector>
            </div>
          </div>
          ...
          <div class="choose-item">
            ...
            <div class="choose-con">
              <staff-selector v-model="member" :options="taffSelectorOptsMb" @change="test"></staff-selector>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <div class="import-progress">
        <div>
          <a-progress type="circle" :percent="importPercent" :width="100" :status="progressStatus"></a-progress>
  ...
  async getPermInfo() {
    ...
    if (res.errcode === 0) {
      ...
      if (authRoles && authRoles.length > 0) {
        this.role = this.roleDisplay = authRoles.map((item: any) => item.bizId);
      }
    } else {
      console.error(res);
    }
