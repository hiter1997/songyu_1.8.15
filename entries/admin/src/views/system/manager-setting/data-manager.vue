<template>
  <div class="data-manager">
    ...
    <div class="data-manager__content">
      <p class="content__tips">
        数据管理员：可对前台应用下的列表数据管理，
        管理设置的应用管理范围下，
        组织管理范围内的人创建的数据
      </p>
      ...
      >
        ...
        <p slot="departmentName" slot-scope="text, record" class="text-ellipsis">{{ text }}</p>
        <p slot="departments" slot-scope="text, record" class="text-ellipsis">{{ text }}</p>

        <p slot="username" slot-scope="text, record" class="text-ellipsis">
        ...
        <p slot="appPackages" slot-scope="text, record" class="text-ellipsis">{{ text }}</p>
      </a-table>
    </div>
    ...
    <add-data-manager v-if="visible" :visible="visible" @submit="submit" @cancel="cancel" :id="id" />
  </div>
</template>
<script lang="ts">
