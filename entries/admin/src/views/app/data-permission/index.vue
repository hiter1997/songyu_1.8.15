...
export default class DataPermission extends Vue {
  ...
  async getDataItems() {
    if (this.dataItemList.length === 0) {
      const bizSchemaCode = this.$route.params.bizSchemaCode;
      this.setBizSchemaCode(bizSchemaCode);
      await this.getDataItemListX();
    }
    const sourceDataItems = this.dataItemList.filter((item: any) => !item.defaultProperty && item.published);
