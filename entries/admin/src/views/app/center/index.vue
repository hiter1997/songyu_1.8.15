<template>
  <div class="apps" v-show="listLoaded" @scroll="contenScroll" ref="apps">
    <div v-if="!appList || !appList.length" class="apps-empty">
      ...
      <a-button type="primary" class="add-app" @click="onAddApp" v-if="isAdmin">
        <a-icon type="plus" />
        {{ $t("languages.Apps.AddApp") }}
      </a-button>
    </div>
    <div v-else class="apps-list" :style="`width: ${wrapWidth}px;`">
      <div class="apps-list-opera">
        ...
        <a-button type="default" class="import-app add-app" @click="importApp">
          <a-icon type="upload" class="app-icon-plus" />
          导入应用
        </a-button>
        <span
          class="search-size"
        ...
        >
          ...
          <a-tooltip placement="top" v-if="showTootip(item.displayName)">
            ...
            <p
              ...
              v-hight-light="{ keyWords: searchValue, value: item.displayName }"
            ></p>
          </a-tooltip>
          <p
            class="content-item-name"
  ...
  onSearch() {
    ...
    if (this.searchValue) {
      this.computedAppList.forEach((app: any) => {
        if (app.displayName.indexOf(this.searchValue) !== -1) {
