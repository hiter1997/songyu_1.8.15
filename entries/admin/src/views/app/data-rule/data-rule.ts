...
import BizModelsSelect from '@/components/global/biz-models-selector/index.vue'
...
export default class DataRuleHandler extends Vue {
   ...
   */
  async selectModel(data:any, theModel?:any) {
    let modeInfo:any = {};
    let code = '';
    let selectTrace = '';
    if (data instanceof Array) {
      ...
      if (theModel) {
        modeInfo.title = theModel.title;
      }
      
    } else {
      const l = data.selectTrace.indexOf('-');
      code = data.code;
      if (l > -1) {
        selectTrace = data.selectTrace.slice(0, l);
      } else {
        selectTrace = data.selectTrace;
      }
      modeInfo =  data;
    }
  //  try {
  ...
  //  }
    ...
    if (res.errcode === 0) {
      ...
      this.getUpDateObjLists(modeInfo, res.data);
      if (this.mode === DataRuleType.OperationMode.Add) {
  ...
  */
  addDataAction(isSetDefault:boolean) {
    if (!isSetDefault && this.UpDateObjIsSheet && this.isShowActionSysDataItems && !this.validateMainScope()) {
  ...
  */
  ...
  // 判断是否可提交
    ...
    if (this.targetTableCode === '' && this.isMain) {
