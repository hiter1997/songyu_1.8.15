...
export default class Workflow extends Vue {
    ...
    if (len) {
      for(let i = 0; i < len; i++) {
        ...
        if(modeCodes.includes(nodeList[i].nodeCode)) {
          this.$message.error(`节点编码 ${nodeList[i].nodeCode} 重复，请检查！`);
          return false;
        }
  ...
  _verifyLine(params:any) {
    ...
    const logicNode = node.filter(res => {
      return res.nodeType === NodeType.LogicVerify;
    });
    for (let i = 0; i < logicNode.length; i++) {
      const code = logicNode[i].nodeCode;
      var nodeInfo = `【${logicNode[i].nodeName}[${logicNode[i].nodeCode}]】`;
  ...
  _verifyActivities() {
    for (let i = 0; i < this.activities.length; i++) {
      ...
      var nodeInfo = `【${activeNode.nodeName}[${activeNode.nodeCode}]】`;
      switch (verifyCode) {
        case "START":
        case "END":
          break;
        ...
        case "DELETE":
          ...
          break;
        case "MESSAGE":
          var { receiver, title, content } = activeNode;
          ...
          break;
        case "LOGIC_VERIFY":
          var { verifySchemaCode, targetSchema } = activeNode;
          if (!verifySchemaCode) {
...
<style lang="less">
  ...
  .property-bind-biz-method,.data-action-modal-content,.property-execute-condition,.property-judgement-condition,.property-timing-task{
    .delete{
      position: relative;
      top: 6px;
    }
