...
export default class ActivityDragBar extends Vue implements H3AvtivityDraggable {
  ...
  onDragover(evt: any) {
    ...
    if (flag.endActivity && flag.endActivity.nodeCode !== this.instance.nodeCode) {
      ...
      if (!_duplicate) {
        ...
        if(flag.startActivity && (flag.startActivity as any).nodeType === NodeType.LogicVerify ) {
          ...
          if (lines.length === 1) {
            ...
            if (theLine.routeCondition && theLine.routeCondition === 1 && flag.endActivity.nodeCode !== 'End') {
              return
            }
          }
  ...
  onDragend() {
    ...
    this.$nextTick(() => {
      if (this.resultLine && this.resultLine.endActivity) {
        ...
        if(this.resultLine.startActivity && (this.resultLine.startActivity as any).nodeType === NodeType.LogicVerify ) {
          ...
          const startCode = this.resultLine.startActivity;
            ...
            if (lines.length === 0) {
              const endp = this.activities.find(res => res.nodeType === NodeType.End)
              
              if (this.resultLine.endActivity && (this.resultLine.endActivity as any).nodeType !== NodeType.End) {
