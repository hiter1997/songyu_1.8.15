<template>
  <div class="div-height">
    <div class="input-modal" @click="modalShow">
      ...
      <a-tooltip slot="suffix" title="定时设置">
    ...
    >
      ...
      <div class="content">
        <a-row>
          <a-col :span="2"><span class="required">执行时间</span></a-col>
        ...
        <a-row>
          <a-col :span="2"><span>执行方式</span></a-col>
        ...
        <a-row v-show="formData.executeType === 'POLLING'">
          <a-col :span="2"><span style="position:relative;top:4px">轮询间隔</span></a-col>
          <a-col :span="22" class="flex_col margin-left">
            <a-select class="wd_80" v-model="formData.hour">
              <a-select-option 
                v-for="item in 24" 
                :key="item"
                :value="item-1"
            ...
            <a-select class="wd_80" v-model="formData.minute">
              <a-select-option 
                v-for="item in 60" 
                :key="item"
                :value="item-1"
            ...
            <a-select class="wd_80" v-model="formData.second">
              <a-select-option 
                v-for="item in 60" 
                :key="item"
                :value="item-1"
        ...
        <a-row v-show="formData.executeType === 'CYCLE'">
          <a-col :span="2"><span>执行周期</span></a-col>
        ...
        <a-row>
          <a-col :span="2"><span>数据源</span></a-col>
          <a-col :span="22" class="margin-left">
            <a-radio-group 
              name="radioGroup" 
            ...
            <a-row v-for="(row,filterIndex) in filtersList" :key="filterIndex">
              <a-col :span="6">
                ...
                <data-item-select
                  ...
                  :disabled="false"
                  @change="(val) => { onDataItemChange(row.dataItem, filterIndex);}"
                ></data-item-select>
              </a-col>

              <a-col :span="5">
                <a-select class="wd_150" v-model="row.maturityFunctionType">
                  <a-select-option 
              ...
              <a-col :span="6">
                <staff-selector 
                  ...
                  v-if="getDataPropertyType(row.dataItem) === DataItemType.StaffSelector"
                  class="wd_180" 
                >
                </staff-selector>
                <!-- 逻辑 -->
                <div v-else-if="getDataPropertyType(row.dataItem) === DataItemType.Logic">
                  <a-select 
                    class="wd_180"
                    v-model="row.value"
                  >
                ...
                <div v-else>
                  <a-input 
                    v-if="getDataPropertyType(row.dataItem) === DataItemType.Number"
                    type="number"
                    class="wd_180"
                    v-model="row.value" 
                  />
                  <a-input 
                    v-else
                    class="wd_180"
                    @change="(val) => { onValueChange(val,row, row.dataItem) }" 
                    v-model="row.value" 
                  />
                </div>
              </a-col>
              <a-col :span="6">
                ...
                >
                  <a-select-option 
                    v-for="(item, index) in row.unit.list"
                    :key="index"
                    :value="item.key"
        ...
        <div v-show="formData.dataSourceType === 'BIZ_SERVICE'">
          ...
          <div class="server-methods">
            ...
            >
              ...
              <a-col class="col" :span="5">{{ item.serviceMethodParameterCode }}</a-col>
              <a-col class="col" :span="5">{{ item.propertyName }}</a-col>
              <a-col class="col" :span="6">{{ getCodeType(item.codeType) }}</a-col>
...
export default class propertyTimingTask extends Mixins(PropertyComponent) {
    ...
    if(value && value.startDate){
      ...
      if(value.dataSourceType === 'GET_LIST'){
        value.schedulerFilterConditions.forEach((s)=>{
          let source: any = this.dataItems.find((d: any) => d.code === s.dataItem);
          if(!source){
            return;
          }
  ...
  handleOk(): void {
    if(!this.startDate){
      this.$message.error('执行时间未填写');
      return;
    }
    if(this.formData.executeType === 'CYCLE' && this.formData.cycleType === 'CUSTOM' && !this.formData.execCycle){
    ...
    if(this.formData.dataSourceType === 'GET_LIST'){
      this.filtersList.forEach((f) =>{
        const source:any = this.dataItems.find((item:any) => f.dataItem === item.code);
        if(!f.dataItem || !f.maturityFunctionType || f.value === ''){
          switchBtn = true;
        }
        if(source && source.propertyType === DataItemType.Number && f.unit.value === 'HOUR'){
    ...
    if(this.formData.dataSourceType === 'BIZ_SERVICE'){
      const dataSourse: any = this.inputMappings.find((i)=>{ return i.bizCode === ''});
      if(dataSourse){
  ...
  getFilterConditionList(row: any){
    const source:any = this.dataItems.find((item:any) => row.dataItem === item.code);
    if(!source){
      return this.maturityFunctionList;
    }
    switch(source.propertyType){
      case DataItemType.ShortText:
        return this.maturityFunctionList.filter((m) =>{ return  (m.index > 2 && m.index < 7) });
        break;
      case DataItemType.Number:
        return this.maturityFunctionList.filter((m) =>{ return  ((m.index > 2 && m.index < 5) || (m.index > 6 && m.index < 11)) });
        break;
      case DataItemType.Date:
        if(!row.unit.value){
          row.unit.value = 'HOUR';
        }
        return this.maturityFunctionList.filter((m) =>{ return (m.index > 0 && m.index < 3) });
        break;
      case DataItemType.Logic: 
      case DataItemType.StaffSelector:
        return this.maturityFunctionList.filter((m) =>{ return m.index === 3 });
        break;
    }
  }
  ...
  getDataPropertyType(code: string){
    const dataSource = this.dataItems.find((d)=>{ return d.code === code});
    if(!dataSource){
      return ''
    }
    ...
    if (method) {
      ...
      if(data){
        this.inputMappings.forEach((input: any)=> {
          let inputSource: any = data.methodMapping.inputMappings.find((d: any) => d.serviceMethodParameterCode === input.serviceMethodParameterCode);
          if(inputSource){
        ...
        this.outputMappings.forEach((output: any)=> {
          let outputSource: any = data.methodMapping.outputMappings.find((d: any) => d.serviceMethodParameterCode === output.serviceMethodParameterCode);
          if(outputSource){
