<template>
  <div class="div-height">
    <div class="input-modal" @click="modalShow">
      ...
      <a-tooltip slot="suffix" title="数据条件">
        <a-icon type="ellipsis" />
      </a-tooltip>
    </div>
    <a-modal wrapClassName="property-data-filter" :visible="visible" @ok="handleOk" @cancel="modalHide" okText="保存" cancelText="取消">
      <template slot="title">
        <div>
          数据条件
      ...
      <div class="content">
        <a-select style="width: 292px" defaultValue="1" v-model="dataConditionJoinType">
          <a-select-option key="ALL">所有数据</a-select-option>
        ...
        <div v-if="dataConditionJoinType!=='ALL'">
          <div class="filters" v-show="filtersList.length>0">
            ...
            <a-row v-for="(row,index) in filtersList" :key="index" style="margin-top:5px">
              <a-col :span="8">
                <data-item-select v-model="row.currentSchemaDataItem" :list="fieldList" :systemFilterCtl="{code: ['workflowInstanceId','modifier' , 'ownerDeptQueryCode', 'sortKey', 'id', 'parentId']}" :bizFilterCtl="{ type: [10, 8, 6, 7]}" :disabled="false" @change="(val) => { dataItemChange(val, index);}"></data-item-select>
              </a-col>

              <a-col :span="6">
                <a-select class="data-filter" v-model="row.ruleDataConditionType" placeholder="请选择" @change="operatorChange(row.ruleDataConditionType,index)">
                ...
                <template>
                  <div class="item-box">
                    ...
                    <div v-if="row.type === 2 &&row.visible">
                      <a-date-picker style="width: 178px" @change="dateChange($event,index)" placeholder="请选择时间" locale={locale} format="YYYY-MM-DD HH:mm:ss" :showTime="{ defaultValue: moment(row.value, 'HH:mm:ss',index) }" :defaultValue="moment(row.value, 'YYYY-MM-DD HH:mm:ss',index)" />
                    </div>

                    <div v-if="row.type === 3&&row.visible">
                      <a-input type="number" style="width: 178px" v-model="row.value" placeholder="请输入"></a-input>
                    </div>
                    ...
                    <div style="width: 178px" v-if="row.type === 5&&row.visible">
                      <a-select v-model="row.value" style="width: 178px" :getPopupContainer="getPopupContainer">
                        <a-select-option value="true">true</a-select-option>
                        <a-select-option value="false">false</a-select-option>
                      </a-select>
                    </div>

                    <div style="width: 178px;" v-if="row.type === 6 && row.code !== 'sequenceStatus'&&row.visible">
...
export default class propertyDataFilter extends Mixins(PropertyComponent) {
  ...
  moment(val: string, f: string, index: number) {
    if (!val) {
      let d = new Date();
  ...
  handleOk() {
    // 如果选择是所有数据则保存空数组值
    this.dataConditionJoinType === "ALL" ? this.filtersList = [] : this.filtersList;
    this.value = {
    ...
    if (this.dataConditionJoinType !== "ALL") {
      let errorMsg: string = "";
      if (this.filtersList.length === 0) errorMsg = "数据条件不成立";
      else {
  ...
  dataItemIsExist() {
    this.filtersList.forEach((item: any, index: number) => {
      const dataItem = this.fieldList.find((field: any) => field.code === item.currentSchemaDataItem);
      if (!dataItem) {
  ...
  setValueCtrl(item: any, index: number,type: boolean = false) {
    ...
    if (item.propertyType === DataItemType.StaffSelector)
      currentRowData.conditionTypeList.reverse();

    // 当数据项改变时选中默认操作符
    type && (currentRowData.ruleDataConditionType =
  ...
  tiggerFilterOf(type: number, code: string) {
    ...
    switch (type) {
      ...
      case DataItemType.StaffSelector:
        if (code !== "creater" && code !== "createdDeptId" && code !== "owner" && code !== "ownerDeptId") {
