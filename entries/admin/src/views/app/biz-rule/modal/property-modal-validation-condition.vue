<template>
  <div class="div-height">
    <div class="input-modal" @click="modalShow">
      ...
      <a-tooltip slot="suffix" title="验证条件">
        <a-icon type="ellipsis" />
      </a-tooltip>
    </div>
    <a-modal title="验证条件" cancelText="取消" okText="确定" v-model="visible" @ok="modalHide" wrapClassName="validationCondition" width="810px">
      <div class="content">
        <div>
          <span style="padding-right:5px">满足以下</span>
          <a-select defaultValue="AND" v-model="dataConditionJoinType">
        ...
        <div class="filters">
          ...
          <a-row v-for="(row,index) in filtersList" :key="index" style="margin-top:5px">
            <a-col :span="5">
              <data-item-select v-model="row.targetSchemaDataItem" :list="targetFieldList" :systemFilterCtl="{code: ['workflowInstanceId','ownerDeptQueryCode', 'sortKey','modifier']}" :disabled="false" @change="(val) => { targetDataItemChange(val, index);}"></data-item-select>
            </a-col>
            <a-col :span="4">
              <template>
                <div>
                  <a-select class="validition-type" defaultValue="SINGLE" v-model="row.verifyType" placeholder="请选择" @change="conditionChange(row.verifyType,index)">
                    <a-select-option key="SINGLE" value="SINGLE">单值比较</a-select-option>
                    <a-select-option key="EXIST" value="EXIST" :disabled="row.targetDataItemDefaultProperty|| row.propertyType === 4">是否存在</a-select-option>
                    <a-select-option :disabled="validModelType ==='main'|| row.propertyType !== 2" key="TOTAL" value="TOTAL">汇总比较</a-select-option>
                  </a-select>
                </div>
              </template>
            </a-col>
            <a-col :span="4">
              <a-select v-if="row.verifyType!='EXIST'" class="data-filter" v-model="row.conditionType" placeholder="请选择" @change="operatorChange(row.conditionType,index)">
                <a-select-option v-for="(type,index) in row.conditionTypeList" :value="type.code" :key="index">{{ type.name }}</a-select-option>
              </a-select>
            </a-col>
            <a-col :span="5">
              <data-item-select v-if="row.verifyType!='EXIST'" v-model="row.value" :list="row.currentFieldList" :systemFilterCtl="{code: ['workflowInstanceId', 'ownerDeptQueryCode', 'sortKey','modifier']}" :disabled="false" @change="(val) => { currentDataItemChange(val, index);}"></data-item-select>
            </a-col>
            <a-col :span="4">
              <div>
...
export default class PropertyValidationCondition extends Mixins(
  ...
  dataItemIsExist() {
    this.filtersList.map((item: any) => {
      const verifyItem = this.currentFieldList.filter((current: any) => current.code === item.value);
  ...
  conditionChange(val: any, index: number) {
    ...
    if (val === "TOTAL") {
      ...
      if (verifyModal && verifyModal.includes(".")) {
        ...
        this.filtersList[index].currentFieldList = this.filtersList[index].currentFieldList.filter((res: any) => res.code.includes("."))
      }
    }else{
      ...
      case DataItemType.ShortText:
        if (current.code === SystemDataItemCode.Id) {
          ...
          Arry = this.currentFieldList.filter(val => {
            return (
              val.propertyType === DataItemType.ShortText ||
              val.code.includes("id") ||
              val.code.includes("name") ||
              val.code.includes("sequenceNo") ||
              val.code.includes("sequenceStatus")
            );
          });

          if (
            current.code === SystemDataItemCode.SequenceStatus ||
            current.code === SystemDataItemCode.SequenceNo
          ) {
            Arry = Arry.filter(
              val => !val.code.includes("id") && !val.code.includes("parentId")
            );
            if (this.validModelType === "main") {
              current.code === SystemDataItemCode.SequenceStatus
                ? (Arry = Arry.filter(val => !val.code.includes("sequenceNo")))
                : (Arry = Arry.filter(
                  val =>
                    !val.code.includes("sequenceNo") &&
                    !val.code.includes("sequenceStatus")
          ...
          Arry = this.currentFieldList.filter(
            val =>
              (val.relativeCode === current.relativeCode &&
                val.propertyType === DataItemType.RelevanceForm) ||
              val.code.includes("parentId") || val.code === "id"
          );
        }
        break;
    }
