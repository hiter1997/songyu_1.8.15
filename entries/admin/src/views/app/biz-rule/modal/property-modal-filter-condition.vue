<template>
  <div class="div-height">
    <div class="input-modal" @click="modalShow">
      ...
      <a-tooltip slot="suffix" :title="title">
    ...
    <a-modal
      ...
      <div class="centent">
        <a-select style="width: 292px" v-model="ruleConditionJoinType">
          ...
          <a-row v-for="(row,index) in filtersList.main" :key="index" style="margin-top:5px">
            <a-col :span="7">
              <data-item-select
                ...
                :disabled="false"
                @change="(val) => { targetDataItemChange(val, index,'main');}"></data-item-select>
            </a-col>
            ...
            <a-col :span="3">
              <a-select
                ...
                <a-select-option v-for="(type,index) in row.conditionTypeList" :value="type.code" :key="index">{{ type.name }}</a-select-option>
              </a-select>
            </a-col>
            <a-col :span="6">
              <a-select style="width: 180px" v-model="row.ruleValueType" @change="valueTypeChange('main',row.ruleValueType,index)">
                <template v-for="ruleValueTypeItem in row.ruleValueTypeList">
                  <a-select-option :value="ruleValueTypeItem.id" :key="ruleValueTypeItem.id">{{ ruleValueTypeItem.name }}</a-select-option>
                </template>
              </a-select>
            </a-col>
            <a-col :span="7">
              <div v-if="row.ruleValueType =='DYNAMIC'">
                <data-item-select
                  ...
                  :bizFilterCtl="{ type: [10, 8, 6, 7]}"
                  :disabled="false"
                  @change="(val) => { currentDataItemChange(val, index,'main');}"></data-item-select>
              </div>
        ...
        <!--子表条件-->
        <div class="filters" v-if="trigerObject.trigerType===1">
          <a-row style="margin-top:15px">
          ...
          <a-row v-for="(row,index) in filtersList.child" :key="index" style="margin-top:5px">
            <a-col :span="7">
              <data-item-select
                ...
                :systemFilterCtl="{code: ['workflowInstanceId', 'ownerDeptQueryCode', 'sortKey', 'modifier']}"
                :disabled="false"
                @change="(val) => { targetDataItemChange(val, index,'child');}"></data-item-select>
            </a-col>
            ...
            <a-col :span="6">
              <a-select style="width: 180px" v-model="row.ruleValueType" @change="valueTypeChange('child',row.ruleValueType,index)">
...
export default class PropertyFilterCondition extends Mixins(PropertyComponent) {
  ...
  moment(val: string, f: string, index: number, type: string) {
    if (!val) {
      let d = new Date();
  ...
  async initModal() {
    ...
    if (this.controller.value && this.controller.value.filterCondition) {
      ...
      if (Array.isArray(value.mainTableConditions)) {
        value.mainTableConditions.length === 0 ? this.filtersList.main = [] : this.filtersList.main = BizRule.deepCopy(value.mainTableConditions);
        if (this.filtersList.main.length > 0) {
          this.filtersList.main.forEach((res, index) => {
            this.targetDataItemChange(res.targetSchemaDataItem, index, 'main', true);
          });
        }
      }
      if (Array.isArray(value.childTableConditions)) {
        value.childTableConditions.length === 0 ? this.filtersList.child = [] : this.filtersList.child = BizRule.deepCopy(value.childTableConditions);
        if (this.filtersList.child.length > 0) {
          this.filtersList.child.forEach((res, index) => {
            this.targetDataItemChange(res.targetSchemaDataItem, index, 'child', true);
          });
        }
      }
  ...
  removeNotExistItem(type: string) {
    this.filtersList[type].map((item: any, index: number) => {
      ...
      const targetSheets = this.targetDataItems.filter(val => val.propertyType === DataItemType.Sheet);

      targetSheets.map((sheet: any) => {
        this.targetDataItems.push(...sheet.subSchema.properties);
      })
      targetdataItem = this.targetDataItems.filter(val => val.code === item.targetSchemaDataItem);
      if (targetdataItem && targetdataItem.length === 0) {
        ...
        if (!item.currentSchemaDataItem.includes('.')) {
          dataItem = this.dataItems.filter((val: any) => val.code === item.currentSchemaDataItem);
        }
        else {
          const sheets = this.dataItems.filter((data: any) => data.propertyType === DataItemType.Sheet);
          ...
          if (code) {
            dataItem = sheetDataItems.filter((val: any) => val.code.includes(code));
          }
        }
        if (dataItem && dataItem.length === 0) {
          const data = item.currentFieldList.find((val: any) => val.code === item.currentSchemaDataItem);
          const dataIndex = item.currentFieldList.indexOf(data);
          (dataIndex > -1) && this.filtersList[type][index].currentFieldList.splice(dataIndex, 1);
    ...
    if (res && res.errcode === 0) {
      ...
      if (this.trigerObject.trigerType === 0) {
        ...
        if (
          ...
          console.log(this.targetFieldList.child);
          // .map(item => {
          //   item.copyCode = item.code;
          //   item.code = `${vals[1]}.${item.code}`;
          //   return item;
          // });
        }
        ...
        this.currentFieldList.main = this.dataItems.filter(
          (d: any) =>
            d.code !== vals[1] && d.propertyType !== DataItemType.Attachment
        );
        ...
        if (
          ...
          childData.subSchema.properties.map((c: any) => {
            !c.code.includes(".") ? (c.code = `${vals[0]}.${c.code}`) : c.code;
            if (!c.code.includes("sortKey"))
              this.currentFieldList.main.push(c);
          });
        }
      }
    } else {
  ...
  modalOk() {
    ...
    Object.getOwnPropertyNames(value.mainTableConditions).length === 0 ? value.mainTableConditions = [] : value.mainTableConditions;
    Object.getOwnPropertyNames(value.childTableConditions).length === 0 ? value.childTableConditions = [] : value.childTableConditions;
  ...
  setRuleValueTypeList(targetItem, index, type, isInit) {
    ...
    if (targetItem.code === SystemDataItemCode.Id ||
      [
      ...
      this.filtersList[type][index].ruleValueTypeList = this.ruleValueTypeList.filter(
        val => val.id === "DYNAMIC"
      );
      return;
    } else if (
  ...
  setCurrentFieldList(targetDataItem, index, type) {
    ...
    switch (targetDataItem.propertyType) {
      case DataItemType.ShortText:
        // 子表业务对象父类Id
        if (targetDataItem.code === SystemDataItemCode.ParentId) {
          // 获取子表schemaCode
          const parentObj = this.targetDataItems.find(
            val => val.code === targetDataItem.schemaCode
          );
          if (parentObj) {
            this.filtersList[type][
              index
              ...
              val =>
                (val.propertyType === DataItemType.ShortText ||
                  (val.propertyType === DataItemType.RelevanceForm &&
                    parentObj.schemaCode === val.relativeCode)) &&
                !val.defaultProperty
            );
          }
        } else if (targetDataItem.code === SystemDataItemCode.Id) {
          this.filtersList[type][
            index
          ].currentFieldList = this.currentFieldList.main.filter(
            val =>
              (val.propertyType === DataItemType.ShortText ||
                (val.propertyType === DataItemType.RelevanceForm &&
                  targetDataItem.schemaCode === val.relativeCode)) &&
              !val.defaultProperty
          );
        } else if (targetDataItem.code === SystemDataItemCode.SequenceStatus) {
          if (this.trigerObject.trigerType === 2) {
            this.filtersList[type][index].currentFieldList = this.currentFieldList.main.filter(val => {
              return (
                val.propertyType === DataItemType.ShortText &&
                !val.code.includes(SystemDataItemCode.SequenceNo) &&
        ...
        else if (targetDataItem.code === SystemDataItemCode.SequenceNo) {
          this.filtersList[type][index].currentFieldList = this.currentFieldList.main.filter(val =>
            val.propertyType === DataItemType.ShortText && !val.code.includes(SystemDataItemCode.Id)
            && !val.code.includes(SystemDataItemCode.SequenceStatus)
            && !val.code.includes(SystemDataItemCode.ParentId)
          )
        }
      ...
      case DataItemType.Logic:
        this.filtersList[type][
          ...
          return val.propertyType === targetDataItem.propertyType;
        });
        break;
      // 关联表单需要映射 相同的数据模型
      case DataItemType.RelevanceForm:
        ...
        if (arry.length === 0) {
          // 主表触发主表
          this.filtersList[type][index].currentFieldList = this.currentFieldList[type].filter(
          ...
          if (this.trigerObject.trigerType === 0) {
            this.filtersList[type][index].currentFieldList = this.currentFieldList[type].filter(
              val => (val.propertyType === targetDataItem.propertyType &&
                targetDataItem.relativeCode === val.relativeCode) || val.code.includes(SystemDataItemCode.Id)
            );
          }
          else {
            // 子表触发主表
            this.filtersList[type][index].currentFieldList = this.currentFieldList.main.filter(
              val => (targetDataItem.relativeCode === val.relativeCode && val.propertyType === targetDataItem.propertyType) ||
                val.code === SystemDataItemCode.Id || val.code.includes(SystemDataItemCode.ParentId)
            );
          }
        }
        break;
      case DataItemType.StaffSelector:
        // 创建人部门
        if (
          targetDataItem.code === SystemDataItemCode.CreatedDeptId ||
          targetDataItem.code === SystemDataItemCode.OwnerDeptId
        ) {
          this.filtersList[type][
            index
          ].currentFieldList = this.currentFieldList.main.filter((val: any) => {
            return (
              val.propertyType === DataItemType.StaffSelector &&
              (val.code !== SystemDataItemCode.Creater &&
                val.code !== SystemDataItemCode.Owner &&
                val.code !== SystemDataItemCode.Modifier)
            );
          });
        } else if (!targetDataItem.defaultProperty) {
          this.filtersList[type][
            index
          ].currentFieldList = this.currentFieldList.main.filter((val: any) => {
            return (
          ...
          targetDataItem.code === SystemDataItemCode.Creater ||
          targetDataItem.code === SystemDataItemCode.Owner
        ) {
          this.filtersList[type][
            index
          ].currentFieldList = this.currentFieldList.main.filter((val: any) => {
            return (
              val.propertyType === DataItemType.StaffSelector &&
              val.code !== SystemDataItemCode.CreatedDeptId &&
              val.code !== SystemDataItemCode.OwnerDeptId &&
              val.code !== SystemDataItemCode.Modifier
            );
          });
        } else {
  ...
  // 根据选择的数据项触发对应的值控件
  setValueCtrl(item: any, index: number, type: string, isInit = false, isVauetypeChage = false) {
    ...
    if(!isInit && !isVauetypeChage){
      currentRowData.ruleConditionType = currentRowData.conditionTypeList[0].code;
    }
    // 根据Item值生成对应的控件
  ...
  valueTypeChange(type: string, value: string, index: number) {
    if (value === "EMPTY") {
      ...
      const targetItem: any = this.targetFieldList[type].find(
        (item: any) => item.code === this.filtersList[type][index].targetSchemaDataItem
      );
