  ...
  >
    <div class="activity activity-content">
      <div class="content-wrap">
        ...
        <span class="activity">{{ instance.activityName || instance.name || instance.nodeName }}</span>
    ...
    <template v-if="instance.nodeCode !== 'End'">
      <!-- 节点边缘拖拽产生线条 -->
...
export default class Activity extends Vue implements H3AvtivityDraggable {
  ...
  activityDragEnd(e: any) {
    ...
    this.$nextTick(() => {
      const activity = this.activities.find((item) => item.code === this.instance.code);
