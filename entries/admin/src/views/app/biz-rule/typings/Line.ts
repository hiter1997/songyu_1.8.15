 ...
 */
  ...
  setTextPosition(position?: LINE.point) {
    ...
    let lineLenght = 0;
    this.points.forEach(res => {
        if (prePoint) {
            if (prePoint.x === res.x) {
                let n = (prePoint.y - res.y)
                lineLenght += n > 0 ? n : -n;
            } else {
                let n = (prePoint.x - res.x)
                lineLenght += n > 0 ? n : -n;
            }
        }
        prePoint = res;
    ...
    let lineLenght2 = lineLenght/2;
    lineLenght = 0;
    prePoint = null;

    for (let i = 0; i < _len; i++) {
        let res = this.points[i];
        if (prePoint) {
            if (prePoint.x === res.x) {
                let n = (prePoint.y - res.y)
                lineLenght += n > 0 ? n : -n;
            ...
            if (lineLenght > lineLenght2) {
                _x = (prePoint.x + res.x) /2;
                _y = (prePoint.y + res.y) /2;
    ...
    console.log('d33333333333333', lineLenght)
