<template>
  <div class="workflow">
        ...
        >
          <template slot="content">
            ...
            <div class="popover-footer">
              ...
              <a-button @click="showPopover = false;noMoreTips = false">取消</a-button>
          ...
          <button
            ...
            :class="{'disabled': isDisabled}"
          >
            <i class="icon aufontAll h-icon-all-play-circle-o"></i>
            <span>运行</span>
          </button>
        </a-popover>
        <button
    ...
    <div class="workflow-content">
      ...
      <!-- 流程绘图区 -->
      <div id="designer-wrap" class="designer-wrap" tabindex="0" @keydown="doKeyDown">
        <workflow-designer ref="designers" @buttonDisable="buttonDisable" v-h3-drop/>
      </div>
      <!-- 节点属性/流程属性 -->
      <div @click="closeContextmenu">
        <h3-sider
    ...
    <!-- 流程运行弹窗 -->
    <StartWorkflow v-model="isShowRunModal" :rules="lines" :activities="activities"/>
...
import MultipleProperty from '@/components/apps/workflow/multiple-node/multiActivityProperty.vue';
...
export default class Workflow extends Vue {
  ...
  // 返回按钮是否禁用
  get redoDisabled() {
    return this.traceManager.LastTraceIndex < this.traceManager.TraceStack.length - 1 ? false: true
  }
  ...
  onVisibleChange(visible: boolean) {
    if (visible) {
      const noMore = window.localStorage.getItem(`${this.workflowCode}-runTips`);
      if (!noMore) {
    ...
    this.deleteWorkflow(params).then((res: any) => {
      if (res.errcode === 0) {
        if (this.$route.name === 'workflowDesign' && code === this.$route.params.workflowCode) {
          this.notValid = true;
          this.$router.push(`/apps/model/${this.appCode}/${this.bizSchemaCode}/data`).catch((err: any) => {err});
        }
        this.$message.success(res.errmsg);
        this.getWorkflows();
