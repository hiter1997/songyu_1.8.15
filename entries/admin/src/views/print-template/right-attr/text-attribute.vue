    ...
    <div class="left-item">
      ...
      <div v-show="whichRadio === 1" class="input-text">
        <a-input
          ...
          v-model.lazy="itemAttrs.innerTxt"/>
      </div>
...
export default class LeftAttribute extends Vue {
  ...
  @PrintVuex.Mutation("setPageEleValue") setPageEleValue!: any;
  ...
  txtChange(e: any) {
    let selectionStart = e.srcElement.selectionStart;
    let selectionEnd = e.srcElement.selectionEnd;

    // 输入的是空格时候需要转换成全角
    const str = this.ToDBC(e.target.value);
    e.target.value = str;
    this.itemAttrs.innerTxt = str;
    ...
    // @ts-ignore
    const height: number = document
      .querySelector(".h3-size-rectangle.active")
      .getClientRects()["0"].height;
    // console.log(height,document.querySelector('.h3-size-rectangle.active').getClientRects())
    // @ts-ignore
    ...
    this.setPageEleValue({
      val: this.itemAttrs.innerTxt,
      id: this.itemAttrs.id,
      key: "innerTxt"
    });

    // 设置光标位置
    e.srcElement.setSelectionRange(selectionStart, selectionEnd);
  }

  // 半角转全角
  ToDBC(txtstring: string): string {
    let tmp: string = "";
    const arry: string[] = Object.values(txtstring);
    arry.map((val: any, index: number) => {
      let space = "";
      if (val === " ") {
        if (txtstring.charCodeAt(index) == 32) {
          space = String.fromCharCode(12288);
        } else if (txtstring.charCodeAt(index) < 127) {
          space = String.fromCharCode(txtstring.charCodeAt(0) + 65248);
        }
      } else {
        space = val;
      }
  ...
  inputToggle(str: string) {
    if (str.includes("focus")) {
      this.changeIsFocus(true);
    }
    if (str.includes("blur")) {
      const height: number = (document as any)
        .querySelector(".h3-size-rectangle.active")
        .getClientRects()["0"].height;
      this.itemAttrs.heightValue = height;
      this.setAttrs(this.itemAttrs);
      this.changeIsFocus(false);
    }
  }
  ...
  selectColor(color: string, str: string) {
    ...
    if (str.includes("fillColor")) {
      this.itemAttrs.fillColorValue = color;
    }
    // @ts-ignore
    if (str.includes("lineColor")) {
      this.itemAttrs.borderColorValue = color;
    }
    // @ts-ignore
    if (str.includes("fontColor")) {
      this.itemAttrs.fontColorValue = color;
    }
  }
