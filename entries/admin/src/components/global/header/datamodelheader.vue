...
export default class Datamodel extends Vue {
  ...
  created() {
    ...
    this.getBizmodeltitle().then((res: any) => {
      if (res.errcode === 0) {
        ...
        let msg:string = res.errmsg || '接口请求失败';
    ...
    switch (type) {
      ...
      case "wf":
        ...
        if (this.workflows.length === 0) {
          ...
          this.$router.push({ name: "workflowEmpty" }).catch((err: any) => {err});
        }
        break;
    ...
    formApiOld
      ...
      .then((res: any) => {
        ...
        if (
          ...
          _this.$router.push(
            `/apps/model/${this.appCode}/${this.bizSchemaCode}/data?del_form_code=${code}`
          ).catch((err: any) => {err});
        }
      })
      .catch((e: any) => {
    ...
    this.deleteWorkflow(params).then((res: any) => {
      if (res.errcode === 0) {
        if (
          ...
          this.$router.push(
            `/apps/model/${this.appCode}/${this.bizSchemaCode}/data`
          ).catch((err: any) => {err});
        }
  ...
  onDataItemChange() {
    ...
    if (this.formData && this.formData.name) {
      ...
      form ? (form.name = this.formData.name) : form;
    }
  }
}
</script>
