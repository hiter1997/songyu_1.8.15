...
export default class workflowSelector extends Vue {
  ...
  onLoadData(treeNode: any) {
    ...
    return new Promise((resolve: any) => {
      ...
      const { type = null, code = '' } = treeNode.dataRef.dataRef|| {};
      if (type === 'app') {
        ...
        const ids: string[] = treeNode.dataRef.dataRef.root.split('-');
  ...
  getAppList() {
    ...
    // 工作交接
    const getAppList: any = this.usePage === 'handover' ? appApi.getAllApps : appApi.getAppList;

    getAppList({ isPortal: true }).then((res: Common.Data) => {
  ...
  getBizModels(appCode: string) {
    return appApi.getAppItem({ appCode, isPortal: true }).then((res: Common.Data) => {
  ...
  getWorkflows(schemaCode: string) {
    return workflowApi.getWorkflowList({ schemaCode, isPortal: true  }).then((res: Common.Data) => {
