...
export default {
  ...
  formulaToContent(formulaContent: string, variableMap: VariableMap): string {
    ...
    const content: string = formulaContent.replace(/(\{.+?\})/g, (code: string) => {
  ...
  joinConnection(editorDom: Element, content: string, selection: Selection) {
    ...
    // 4.根据前后的字符进行运算符号追加
    if (
      preChar && !['(', ',', '+'].includes(preChar)
      ||
      !preChar && preNode && preNode.nodeName === formulaNodeName
    ) {
      content = '+' + content;
    }
    ...
    if (
      ...
      ||
      !nextChar && nextNode && nextNode.nodeName === formulaNodeName
    ) {
      content += '+';
    }
