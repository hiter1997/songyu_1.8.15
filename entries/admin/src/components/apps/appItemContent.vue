<template>
  <div class="item-content">
    ...
    <!-- 新建报表弹窗 -->
    <AddReport v-model="showAddReport" mode="add"></AddReport>

    <!-- 导入 -->
    <data-import
      v-if="isShowDataImport"
      :accept="'.xml'"
      :action="uploadUrl"
      ref="dataImport"
...
export default class AppItemContent extends Vue {
  ...
  onImportSuccess(v: boolean) {
    let fileName: string = (this.$refs.dataImport as any).fileName;
    if (fileName && !v) {
      listApi.deleteTemporaryFile({ fileName });
    }
    this.isShowDataImport = v;
    this.getAppItem({ appCode: this.appInfo.appCode });
  }
