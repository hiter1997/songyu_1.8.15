...
export default class ParticipantModal extends Vue {
  ...
  descriptions: Apps.Workflow.FormulaDescription[] = [
    ...
    {
      code: 'OUManager',
      text: 'OUManager(x)，查找部门主管。这里指的是查找（某个部门/某人所在部门）的部门主管。例如，OUManager({Originator})-查找发起人所在部门的部门主管，部门主管在组织架构部门属性中显示；OUManager({"user1"})-查找数据项user1（选人控件类型）所在部门的部门主管；',
    },
    {
      code: 'FindUserByRole',
      text: 'FindUserByRole(x,y)，根据角色查找用户。这里指的是查找管理范围包含x（单个人/单个部门，不支持手动输入人的ID/部门ID）的y(角色编码)角色对应的用户，例如，FindUserByRole({Originator},"caiwu"),查找管理范围包含当前处理人的角色编码为“caiwu”的用户；',
    },
    {
    ...
    {
      code: 'OUManagerOf',
      text: 'OUManagerOf(x,y)，查找指定层级的部门主管。这里指的是以x为基础，向上查找第y级部门的部门主管，X指单个人/单个部门，目前只支持数据项和系统参数作为参数，不支持手动输入用户ID/部门ID。示例：OUManagerOf({Originator},2)：查找到发起人上上级部门的部门主管。'
    },
    {
      code: 'RecurseManagerOf',
      text: 'RecurseManagerOf(x,y)，逐级查找部门主管。这里指的是查找x的部门主管，并递归向上继续查找部门主管，向上查找几级由y定义，X指单个人/单个部门，目前只支持数据项和系统参数作为参数，不支持手动输入用户ID/部门ID。示例：RecurseManagerOf({Originator},2)：查找到发起人的部门主管，并继续向上查找2级部门主管，即查找到3个人，分别为发起人的部门主管、发起人上级部门的部门主管、发起人上上级部门的部门主管。'
    }
  ];
