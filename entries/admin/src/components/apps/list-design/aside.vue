...
export default class Aside extends Vue {
  ...
  setShowfieldList(val: any) {
    this.showfieldList = val
      .map((x: any, i: any) => x)
      .filter((x: any, i: any) => {
        const ignoreType = this.clientType === "APP" ? [6, 7, 8] : [7]; // type代表控件类型(暂时放开附件-- 6，pc端放开子表--8)
        return !ignoreType.includes(x.type);
      });
    // .filter((x:any, i:any) => {
  ...
  setDefaultValue(dataItems: any) {
    ...
    const targetDataItems = dataItems.map((res: any, index: number) => {
      ...
      if (res.propertyType === 8 && res.subSchema && Array.isArray(res.subSchema.properties)) {
        res.subSchema.properties.forEach((child:any) => {
          delete child.id;
          child.propertyCode = child.code;
          this.typeFormat(child.propertyType, child);
        });
        childColumns = res.subSchema.properties.filter((sub:any) => !sub.defaultProperty);
      }
  ...
  typeFormat(propertyType:any, obj:any) {
  ...
  initData(backData: Array<any>, targetData: Array<any>) {
    ...
    backData.forEach((res: any) => {
      let target: any = targetData.find(
        (r: any) => res.propertyCode === r.propertyCode
      );
      if (!target) return;
      ...
      // 子表的子数据项处理
      if (res.propertyType === 8 && Array.isArray(res.childColumns) && Array.isArray(target.childColumns)) {
        res.childColumns.forEach((child:any, i:number) => {
          let childTarget:any = target.childColumns.find((s:any) => s.code === child.propertyCode);
          if (childTarget) {
    ...
    // 连接以 targetData 为主
    sortAfter = targetData.filter(
      (res: any) =>
        !backData.some(item => item.propertyCode === res.propertyCode)
    );
