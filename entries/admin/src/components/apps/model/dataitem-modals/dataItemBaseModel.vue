...
<template>
  <div>
    <FormDrawer :templates="templates" :bizSchemaCode="$route.params.bizSchemaCode"></FormDrawer>
  </div>
</template>
    ...
    return {
      ...
      templates: [
        ...
        },
        {
          ...
          maxLength: 200,
          minLength: 0,
          pattern: new RegExp(/^[^ ].{0,199}$/),
          ruleMsg: '请输入不以空格开头长度不超过200个字符',
      ...
      selectCollection: {
        // 数据项
        type: [{ label: 'rrrrr', value: 'man' }, { label: 'ggggg', value: 'woman' }],
        // 选择业务类型
        businessTypeSelect: [{ label: '业务类型222', value: 'man' }, { label: '业务类型3333', value: 'woman' }]
      }
    };
  }
});
