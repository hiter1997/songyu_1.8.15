...
const mutations = {
  ...
  updateActivityProps(state: any, updatedActivity: WorkflowNamespace.Activity) {
    state.activities.some((activity: WorkflowNamespace.Activity, index: number) => {
  ...
  updateLineProps(state: any, updatedLine: Line) {
    state.lines.some((line: Line) => {
      if (line.ID === updatedLine.ID) { // TODO refactor search update  data 
        line = updatedLine;
        return true;
      }
    });
  ...
  unSelect(state: any, Activity?: WorkflowNamespace.Activity) {
    ...
    if (Activity) {
      ...
      state.activities.some((e: any, i: string) => {
        if(e.activityCode === Activity.activityCode){
          const filterSelectedActivity = state.selectedActivities.filter((s: any, j: string) => e.ID != s.ID);
  ...
  switchToFirst(state: any, Activity: WorkflowNamespace.Activity) {//多个活动圈选，点击启动一个
    if (!Activity) return;
    ...
    if (state.selectedActivities[0].ID !== Activity.ID) {
      ...
      state.selectedActivities.some((e: any, i: string) => {
        if (Activity.ID === e.ID) { // TODO refactor  用filter unshift 
          index = i;
          model = e;
          return true;
        }
  ...
  setWorkflowData(state: any, data: any) {
    ...
    ['id','schemaCode','workflowName',
    'name_i18n','workflowCode','icon',
    'pcOriginate','mobileOriginate','sortKey',
    'workflowVersion','templateType','shortCode',
    'externalLinkEnable',
    // 流程事件 已定义
    'startEventHandler','cancelEventHandler',
    'activateEventHandler', 'endEventHandler'
  ...
  showPropertyPanel(state: any, component: string) {
    ...
    if (component !== '') {
      ...
      const allUser = state.selectedActivities.every((ac:any) => ac.activityType === 'PARTICIPANT');
      if (multi && allUser) {
  ...
  dataToProps(state: any, type?: string) {
    ...
    let workflowCode = _curActivity.workflowCode; // refactor start 
        ...
        (workflowCode.includes('-') ? workflowCode : `workflow-${_curActivity.workflowCode}`) : '';  // refactor end
  ...
  propsToData(state: any) {
    ...
    if (_index !== -1) {
      ...
      state.lines.forEach((line: Line) => {
        if (line.startActivity && line.startActivity.ID === state.curActivityID) {
          line.startActivity = state.activities[_index];
        } else if (line.endActivity && line.endActivity.ID === state.curActivityID) {
   ...
   changeActivityByBatch(state: any, prop: WorkflowNamespace.PropValueByBatch) {
    ...
    state.selectedActivities.forEach((sel:any) => {
      ...
      if (typeof PropValue === 'string') {
        ...
        let item:any = null;
        if (cKey.length > 1) {
          item = sel.propertyPermissions.find((p:any) => p.propertyCode === cKey[0]);
        } else {
          item = sel.propertyPermissions.find((p:any) => p.propertyCode === PropValue.key);
        }
        ...
        if (item && typeof PropValue.value === 'object') {
          Object.entries(PropValue.value).forEach((a:any) => {
            ...
            if (cKey.length > 1 && Array.isArray(item.subPropertyPermissions)) {
              // 子表数据项
              const subItem = item.subPropertyPermissions.find((s:any) => s.propertyCode === cKey[1]);
              subItem[_key] = value;
              return;  
            }
            item[_key] = value;
