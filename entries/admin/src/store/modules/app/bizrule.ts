...
const mutations = {
  ...
  addActivity(state: any, acticitieModel: WorkflowNamespace.Activity) {
    ...
    state.lines.forEach(res => {
      if (res.startActivity && res.startActivity.nodeCode === "$virtualNode") {
        ...
        if (_acticity.nodeType === NodeType.LogicVerify) {
  ...
  updateNodeProps(state: any, data: any) {
    const theNode = state.activities.find(
      node => state.currentActivity.nodeCode === node.nodeCode && state.currentActivity.ID === node.ID
    );
    ...
    if (acticitieModel) {
      if (!acticitieModel.isSelected) {
        ...
        state.activities.forEach((e: any, i: string) => {
          ...
          if (e.nodeCode === acticitieModel.nodeCode && e.ID === acticitieModel.ID) {
            e.isSelected = true;
...
const actions = {
  ...
  onActivityConnect({ state, commit }: any, param: any) {
    ...
    if (startActivity.nodeType === NodeType.LogicVerify) {
    ...
    if(line.startActivity && (line.startActivity as any).nodeType === NodeType.LogicVerify ) {
      ...
      if (lines.length === 1) {
        ...
        if (theLine.routeCondition && theLine.routeCondition === 1 && endActivity.nodeCode !== 'End') {
          return
        }
      }
      ...
      if (lines.length === 0) {
        ...
        if (line.endActivity && (line.endActivity as any).nodeType !== NodeType.End) {
  ...
  setTreeDataAsync({ commit, state }: any, params: any) {
    ...
    switch (indexs.length) {
      ...
      case 2:
        ...
        break;
      case 3:
        temTree[indexs[0]].children[indexs[1]].children[
          indexs[2]
        ].children = tree;
        break;
      default:
