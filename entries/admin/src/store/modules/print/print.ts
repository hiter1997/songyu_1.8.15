...
const mutations = {
  setPageSettingData(state: any, data: any) {
    ...
    sessionStorage.setItem(
      `printStore${data.code}`,
      JSON.stringify(state.pageSettings)
    );
  },
  ...
  setPageEleValue(state: any, { val, id, key }) {
  ...
  setAttrsPicUrl(state: any, { picUrl, id }) {
    state.itemAttrs.picUrl = picUrl;
    for (let page of state.pages) {
      for (let ele of page) {
        if (ele.id === id) {
          ele.picUrl = picUrl;
...
const getters = {
  ...
  getPageSettingData: (state: any) => (code: string) => {
    const sessionData: string | null = sessionStorage.getItem(
      `printStore${code}`
    );
    sessionData === null
      ? (state.pageSettings = null)
      : (state.pageSettings = Object.assign({}, JSON.parse(sessionData)));
