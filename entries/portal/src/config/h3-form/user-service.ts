...
export class DefaultUserService implements renderer.UserService {
  ...
  async getDepartmentsBy(
    ...
    const res = await organizationApi.getOrgTree(deptIds, filterType, sourceType, corpId, excludeCorpId, selectUserIds, workflowInstanceId, activityCode);
    const data = res.data;
  ...
  async getUsersBy(
    ...
    const res = await organizationApi.getUsersTree(deptId, roleId, filterType, sourceType, workflowInstanceId, activityCode);
    const data = res.data;
